...
data_file_path = os.path.join(os.getcwd(), 'files', 'storage-v2.txt') #'files/storage.txt'
...
def home():
    ...
    return render_template('home-v2.html',app_mode=app.config['APP_MODE'],
...
def sqlhome():
    ...
    try:
        ...
        return render_template('return.html', title='Action Not Successful',
            message="somthing went wrong", data={"new_content": new_content,"Status": "Could not load from DB"},
            message_type='fail')
...
def update_content():
    ...
    try:
        ...
        return render_template('return.html', title='Action Successful',
            message="Content saved successfully", data={"Path": data_file_path, "Status": "File created, and Inserted into DB"},
        ...
        f.write(new_content)
        return render_template('return.html',title='Action Successful', 
                message="The file was created but is not in the database.", 
                data= {"Path": data_file_path, "Status": "File created"}, 
                message_type='success' ,
                path=data_file_path)  #jsonify({"message": "file does not exist, so created one, but not in db!","path":data_file_path})
...
@app.route('/about')
def about():
    return render_template('about-v2.html')
